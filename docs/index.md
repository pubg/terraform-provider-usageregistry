...
`transaction_retry_max_attempts` is accepted for backward compatibility but ignored because current storage operations do not use DynamoDB transactions.
...
- `transaction_retry_max_attempts` (Number) Deprecated compatibility setting. Accepted but ignored because current storage operations do not use DynamoDB transactions.
