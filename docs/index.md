...
- `access_key` (String, Sensitive) AWS access key ID to use explicitly. Must be set with `secret_key`.
...
- `profile` (String) AWS shared configuration profile to use explicitly. Conflicts with `access_key`, `secret_key`, and `token`.
